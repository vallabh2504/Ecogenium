...
% Version: 1.1 (Fixed - handles Inf/NaN, white background, colorbar fix)
% =========================================================================
% Based on:
%   - Vehicle mass: 180 kg
...
% You can modify these based on your vehicle's actual specifications
...
motor_efficiency = 0.90;  % 90% typical for brushless DC motors
...
for i = 1:length(speed_ms)
    ...
    % 4. Total power at wheels
    P_total_wheel(i) = P_rolling(i) + P_aero(i) + P_accel(i);

    % 5. Motor output power (accounting for drivetrain losses)
    if P_total_wheel(i) > 0
        P_motor_output(i) = P_total_wheel(i) / drivetrain_efficiency;
    else
        % During coasting/braking, no motor power needed
        P_motor_output(i) = 0;
    end

    % 6. Electrical input power (accounting for motor efficiency)
...
fprintf('  Losses (drivetrain + motor): %.1f Wh (%.1f%%)\n', ...
    total_energy_Wh - (energy_rolling + energy_aero + energy_accel), ...
    (total_energy_Wh - (energy_rolling + energy_aero + energy_accel))/total_energy_Wh*100);
...
subplot(3,2,2);
hold on;
area(time/60, [P_rolling, P_aero, max(P_accel, 0)], 'LineWidth', 0.5);
xlabel('Time (minutes)', 'FontSize', 12);
ylabel('Power (W)', 'FontSize', 12);
title('Power Component Breakdown', 'FontSize', 14, 'FontWeight', 'bold');
legend('Rolling Resistance', 'Aero Drag', 'Acceleration', 'Location', 'best');
grid on;
colormap([0.8 0.4 0; 0 0.4 0.8; 0.8 0 0]);
set(gca, 'Color', 'white');
...
% Subplot 4: Power vs Speed (FIXED colorbar)
subplot(3,2,4);
...
cb = colorbar;
cb.Label.String = 'Time (s)';  % FIXED: Compatible syntax
grid on;
set(gca, 'Color', 'white');

% Subplot 5: Energy per distance segment
subplot(3,2,5);
% Calculate energy per km in sliding window
window_size = 50;
...
subplot(3,2,6);
energy_breakdown = [energy_rolling, energy_aero, energy_accel, ...
    total_energy_Wh - (energy_rolling + energy_aero + energy_accel)];
labels = {'Rolling Resistance', 'Aero Drag', 'Acceleration', 'System Losses'};
...
fprintf('\n** NOTE: Assumed parameters used for calculation **\n');
fprintf('Update the following in the script if you have actual values:\n');
fprintf('  - drag_coefficient (currently %.2f)\n', drag_coefficient);
fprintf('  - frontal_area (currently %.2f m^2)\n', frontal_area);
fprintf('  - rolling_resistance_coef (currently %.4f)\n', rolling_resistance_coef);
fprintf('  - drivetrain_efficiency (currently %.0f%%)\n\n', drivetrain_efficiency*100);

%% ========================================================================
% EXPORT RESULTS
% =========================================================================
% Create results table
results_table = table(time, speed_kmh, speed_ms, rpm, ...
    P_rolling, P_aero, P_accel, P_total_wheel, P_motor_output, P_electrical_input, ...
    cumulative_energy_Wh, ...
    'VariableNames', {'Time_sec', 'Speed_kmh', 'Speed_ms', 'MotorRPM', ...
    'Power_Rolling_W', 'Power_Aero_W', 'Power_Accel_W', 'Power_Wheel_W', ...
